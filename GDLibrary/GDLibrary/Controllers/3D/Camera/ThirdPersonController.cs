...
namespace GDLibrary
{
    public class ThirdPersonController : TargetController
    {
        ...
        private void UpdateFromScrollWheel(GameTime gameTime)
        {
            ...
            float scrollWheelDelta = -this.inputManagerParameters.MouseManager.GetDeltaFromScrollWheel() * gameTime.ElapsedGameTime.Milliseconds;

            //if something changed then update
            if (scrollWheelDelta != 0)
            {
                if (this.inputManagerParameters.KeyboardManager.IsKeyDown(Keys.LeftControl))
                {
                    //todo...add code to change elevation here - 7.2                    
                }
                else
                {
